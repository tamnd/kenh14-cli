...
describes what users get. Pick a format with `-o`, or let kenh14 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kenh14 <command> -o table   # aligned columns for reading
kenh14 <command> -o jsonl   # one JSON object per line, for piping
kenh14 <command> -o json    # a single JSON array
kenh14 <command> -o csv     # spreadsheet friendly
kenh14 <command> -o tsv     # tab-separated
kenh14 <command> -o url     # just the URL column
kenh14 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kenh14 <command> --fields id,title,url
```
...
```bash
kenh14 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kenh14 <command>            # a table, because this is a terminal
kenh14 <command> | wc -l    # JSONL, because this is a pipe
```
